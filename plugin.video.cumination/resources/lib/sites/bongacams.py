...
site = AdultSite('bongacams', '[COLOR yellow]bongacams.com[/COLOR]', 'http://bongacams.com', 'bongacams.gif', 'bongacams', True)
...
def Main():
    ...
    if female:
        site.add_dir('[COLOR yellow]Female[/COLOR]', '{0}female'.format(bu), 'List', '', '')
    if couple:
        site.add_dir('[COLOR yellow]Couples[/COLOR]', '{0}couples'.format(bu), 'List', '', '')
    if male:
        site.add_dir('[COLOR yellow]Male[/COLOR]', '{0}male'.format(bu), 'List', '', '')
    if trans:
        site.add_dir('[COLOR yellow]Transsexual[/COLOR]', '{0}transsexual'.format(bu), 'List', '', '')
    utils.eod()
...
def List(url):
    ...
    for model in model_list:
        ...
        name += ' [COLOR yellow][{}][/COLOR]'.format(age)
        if model['hd_cam']:
