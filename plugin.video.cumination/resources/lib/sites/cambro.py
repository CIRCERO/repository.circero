...
site = AdultSite('cambro', '[COLOR yellow]Cambro[/COLOR]', 'https://www.cambro.tv/', 'cambro.gif', 'cambro')


@site.register(default_mode=True)
def Main():
    site.add_dir('[COLOR yellow]Categories[/COLOR]', '{0}categories/'.format(site.url), 'Categories', site.img_cat)
    site.add_dir('[COLOR yellow]Models[/COLOR]', '{0}models/1/'.format(site.url), 'Models', site.img_cat)
    site.add_dir('[COLOR yellow]Search[/COLOR]', '{0}search/?mode=async&function=get_block&block_id=list_videos_videos_list_search_result&category_ids=&sort_by=&from_videos=01&q='.format(site.url), 'Search', site.img_search)
