...
site = AdultSite('javbangers', '[COLOR yellow]JAV Bangers[/COLOR]', 'https://www.javbangers.com/', 'javbangers.gif', 'javbangers')
...
def Main():
    ...
    site.add_dir('[COLOR yellow]Playlists[/COLOR] [COLOR orange]{}[/COLOR]'.format(sortname), site.url + 'playlists/?mode=async&function=get_block&block_id=list_playlists_common_playlists_list&sort_by={}&from=01'.format(jbsortorder), 'Playlists', site.img_cat, contextm=contextmenu)
    site.add_dir('[COLOR yellow]Search[/COLOR]', site.url + 'search/{0}/', 'Search', site.img_search)
    if not jblogged:
        site.add_dir('[COLOR yellow]Login[/COLOR]', '', 'JBLogin', '', Folder=False)
    elif jblogged:
        ...
        site.add_dir('[COLOR yellow]Logout {0}[/COLOR]'.format(jbuser), '', 'JBLogin', '', Folder=False)
...
def List(url, page=1):
    ...
    if re.search(r'<li\s*class="next"><a', listhtml, re.DOTALL | re.IGNORECASE):
        ...
        site.add_dir('[COLOR yellow]Next Page...[/COLOR] (' + str(npage) + lastp + ')', nurl, 'List', site.img_next, npage)
    utils.eod()
...
def Playlists(url, page=1):
    ...
    if re.search(r'<li\s*class="next"><a', cathtml, re.DOTALL | re.IGNORECASE):
        ...
        if 'from={0:02d}'.format(page) in url:
            ...
            nurl = url
        site.add_dir('[COLOR yellow]Next Page...[/COLOR] (' + str(npage) + lastp + ')', nurl, 'Playlists', site.img_next, npage)
    utils.eod()
