...
site = AdultSite('hentaihaven', '[COLOR yellow]Hentaihaven[/COLOR]', 'https://hentaihaven.org/', 'hh.gif', 'hentaihaven')


@site.register(default_mode=True)
def Main():
    site.add_dir('[COLOR yellow]Categories[/COLOR]', '{0}pick-your-poison/'.format(site.url), 'Categories', site.img_cat)
    site.add_dir('[COLOR yellow]A to Z[/COLOR]', '{0}pick-your-series/'.format(site.url), 'A2Z', '', '')
    site.add_dir('[COLOR yellow]Uncensored[/COLOR]', '{0}ajax.php?action=pukka_infinite_scroll&page_no=1&grid_params=infinite_scroll=on&infinite_page=2&infinite_more=true&current_page=taxonomy&front_page_cats=&inner_grid%5Buse_inner_grid%5D=on&inner_grid%5Btax%5D=post_tag&inner_grid%5Bterm_id%5D=53&inner_grid%5Bdate%5D=&search_query=&tdo_tag=uncensored&sort=date'.format(site.url), 'List', '', '')
...
def A2Z(url):
    ...
    for catpage, name, img, other in match:
        if 'uncensored' in other:
            name = name + " [COLOR yellow]Uncensored[/COLOR]"
