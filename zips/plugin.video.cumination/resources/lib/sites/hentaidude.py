...
site = AdultSite("hentaidude", "[COLOR yellow]Hentaidude[/COLOR]", 'https://hentaidude.com/', "hentaidude.gif", "hentaidude")


@site.register(default_mode=True)
def hentaidude_main():
    site.add_dir('[COLOR yellow]Uncensored[/COLOR]', site.url + 'tag/uncensored/page/1/?orderby=date', 'hentaidude_list', site.img_cat, 1)
    site.add_dir('[COLOR yellow]Search[/COLOR]', site.url + 'page/1/?s=', 'hentaidude_search', site.img_search)
...
def hentaidude_list(url, page=1):
    ...
    for name, video, img, other in match:
        ...
        if 'uncensored' in other.lower() or 'uncensored' in name.lower():
            name += " [COLOR yellow][I]Uncensored[/I][/COLOR]"
