...
try:
    ...
    c.executescript("CREATE TABLE IF NOT EXISTS movie(idMovie, idFile, c00, c01, c02, c03, c04, idSet);")
    c.execute('CREATE UNIQUE INDEX ix_favorites(idMovie,idFile);')
...
@url_dispatcher.register()
def write_file(download, filename):
    ...
    try:
        conn = sqlite3.connect(favoritesdb)
        c = conn.cursor()
        c.execute("SELECT * from favorites where download = %s")
