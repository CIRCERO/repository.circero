...
addon_sys = str(sys.argv[0][2:])
addon = xbmcaddon.Addon()
TRANSLATEPATH = xbmcvfs.translatePath if six.PY3 else xbmc.translatePath
FAVORITES = 'favorites' + '.sqlite3' if six.PY3 else '.sdb'
HOME = TRANSLATEPATH('special://home/')
    
try:
    rootDir = os.path.abspath(os.path.join(os.path.dirname(os.path.realpath(__file__).decode(sys.getfilesystemencoding(), 'ignore')), "..", "..", ".."))
except:
    rootDir = addon.getAddonInfo("path")
resDir = os.path.join(rootDir, 'resources')
imgDir = os.path.join(TRANSLATEPATH(HOME + 'media'), 'images')
aboutDir = os.path.join(resDir, 'about')
profile = os.path.join(TRANSLATEPATH(addon.getAddonInfo('profile')), "..", "..", "..")
profileDir = os.path.join(TRANSLATEPATH('special://temp/cache/'), "..")
databases = os.path.join(os.path.dirname(TRANSLATEPATH('special://database')), 'favorites')
cookiePath = os.path.join(databases, 'cookies.lwp')
favoritesdb = os.path.join(databases, FAVORITES)
customSitesDir = os.path.join(TRANSLATEPATH(profileDir), 'elementum_library')
tempDir = os.path.join(profileDir, '')

cuminationicon = os.path.join(TRANSLATEPATH(imgDir), 'icon.gif' if six.PY3 else 'fanart.gif')
...
def addDownLink(name, url, mode, iconimage, desc='', stream=None, fav='add', noDownload=False, contextm=None, fanart=None, duration='', quality=''):
    ...
    if not fanart:
        fanart = os.path.join(imgDir, 'fanart.gif')
    ...
    contextMenuItems.append(('[COLOR yellow]' + favtext + ' favorites[/COLOR]', 'RunPlugin(' + favorite + ')'))
    if fav == 'del':
        ...
        contextMenuItems.append(('[COLOR yellow]Move favorite to Top[/COLOR]', 'RunPlugin(' + favorite_move_to_end + ')'))
        favorite_move_up = (sys.argv[0]
                            ...
                            + "&duration=" + urllib_parse.quote_plus(duration)
                            + "&quality=" + urllib_parse.quote_plus(quality))
        contextMenuItems.append(('[COLOR yellow]Move favorite Up[/COLOR]', 'RunPlugin(' + favorite_move_up + ')'))
        favorite_move_down = (sys.argv[0]
                              ...
                              + "&duration=" + urllib_parse.quote_plus(duration)
                              + "&quality=" + urllib_parse.quote_plus(quality))
        contextMenuItems.append(('[COLOR yellow]Move favorite Down[/COLOR]', 'RunPlugin(' + favorite_move_down + ')'))

    if not noDownload:
        contextMenuItems.append(('[COLOR yellow]Download Video[/COLOR]', 'RunPlugin(' + dwnld + ')'))
    settings_url = (sys.argv[0]
                    + "?mode=" + str('utils.openSettings'))
    contextMenuItems.append(
        ('[COLOR yellow]Addon settings[/COLOR]', 'RunPlugin(' + settings_url + ')'))
    setview = (sys.argv[0]
               + "?mode=" + str('utils.setview'))
    contextMenuItems.append(
        ('[COLOR yellow]Set this view as default[/COLOR]', 'RunPlugin(' + setview + ')'))
...
def addDir(name, url, mode, iconimage=None, page=None, channel=None, section=None, keyword='', Folder=True, about=None,
    ...
    if about:
        about_url = (sys.argv[0]
                     ...
                     + "&custom=" + str(custom))
        contextMenuItems.append(
            ('[COLOR yellow]About site[/COLOR]', 'RunPlugin(' + about_url + ')'))
    if len(keyword) >= 1:
        keyw = (sys.argv[0]
                + "?mode=" + str('utils.delKeyword')
                + "&keyword=" + urllib_parse.quote_plus(keyword))
        contextMenuItems.append(('[COLOR yellow]Remove keyword[/COLOR]', 'RunPlugin(' + keyw + ')'))
    if list_avail:
        ...
        list_url = (sys.argv[0]
                    ...
                    + "&img=" + urllib_parse.quote_plus(iconimage)
                    + "&name=" + urllib_parse.quote_plus(name))
        contextMenuItems.append(('[COLOR yellow]%s[/COLOR]' % list_item_name, 'RunPlugin(' + list_url + ')'))
    if listitem_id:
        move_listitem_url = (sys.argv[0]
                             + "?mode=" + str('favorites.move_listitem')
                             + "&listitem_id=" + str(listitem_id))
        contextMenuItems.append(('[COLOR yellow]Move item to ...[/COLOR]', 'RunPlugin(' + move_listitem_url + ')'))
        listitem_url = (sys.argv[0]
                        + "?mode=" + str('favorites.remove_listitem')
                        + "&listitem_id=" + str(listitem_id))
        contextMenuItems.append(('[COLOR yellow]Remove from list[/COLOR]', 'RunPlugin(' + listitem_url + ')'))
        moveupitem_url = (sys.argv[0]
                          + "?mode=" + str('favorites.moveup_listitem')
                          + "&listitem_id=" + str(listitem_id))
        contextMenuItems.append(('[COLOR yellow]Move item Up[/COLOR]', 'RunPlugin(' + moveupitem_url + ')'))
        movedownitem_url = (sys.argv[0]
                            + "?mode=" + str('favorites.movedown_listitem')
                            + "&listitem_id=" + str(listitem_id))
        contextMenuItems.append(('[COLOR yellow]Move item Down[/COLOR]', 'RunPlugin(' + movedownitem_url + ')'))

    if custom_list:
        editlist_url = (sys.argv[0]
                        + "?mode=" + str('favorites.edit_list')
                        + "&rowid=" + str(url))
        contextMenuItems.append(('[COLOR yellow]Edit name[/COLOR]', 'RunPlugin(' + editlist_url + ')'))
        dellist_url = (sys.argv[0]
                       + "?mode=" + str('favorites.remove_list')
                       + "&rowid=" + str(url))
        contextMenuItems.append(('[COLOR yellow]Remove list[/COLOR]', 'RunPlugin(' + dellist_url + ')'))
        moveuplist_url = (sys.argv[0]
                          + "?mode=" + str('favorites.moveup_list')
                          + "&rowid=" + str(url))
        contextMenuItems.append(('[COLOR yellow]Move list Up[/COLOR]', 'RunPlugin(' + moveuplist_url + ')'))
        movedownlist_url = (sys.argv[0]
                            + "?mode=" + str('favorites.movedown_list')
                            + "&rowid=" + str(url))
        contextMenuItems.append(('[COLOR yellow]Move list Down[/COLOR]', 'RunPlugin(' + movedownlist_url + ')'))
...
def searchDir(url, mode, page=None, alphabet=None):
    if not alphabet:
        addDir('[COLOR yellow]Add Keyword[/COLOR]', url, 'utils.newSearch', cum_image('cum-search.gif'), '', mode, Folder=False)
        addDir('[COLOR yellow]Alphabetical[/COLOR]', url, 'utils.alphabeticalSearch', cum_image('cum-search.gif'), '', mode)
        if addon.getSetting('keywords_sorted') == 'true':
            addDir('[COLOR yellow]Unsorted Keywords[/COLOR]', url, 'utils.setUnsorted', cum_image('cum-search.gif'), '', mode, Folder=False)
        else:
            addDir('[COLOR yellow]Sorted Keywords[/COLOR]', url, 'utils.setSorted', cum_image('cum-search.gif'), '', mode, Folder=False)
    ...
    try:
        ...
        for (keyword,) in c.fetchall():
            ...
            addDir(name, url, mode, cum_image('cum-search.gif'), page=page, keyword=keyword)
    except:
        pass
    conn.close()
    eod()
